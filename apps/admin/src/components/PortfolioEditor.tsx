...
export function PortfolioEditor({ portfolio }: { portfolio: AdminPortfolio }) {
  ...
  const save = () => startTransition(async () => {
    const result = await updatePortfolioContent(current, content);
    if (!result.ok) {
  ...
  return (
    <section className="space-y-8">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Certifications</CardTitle></CardHeader><CardContent className="space-y-5">{content.certifications.map((item, index) => <div key={item.id ?? index} className="rounded-lg border border-border p-4"><label className="block text-sm font-medium">Name<input className={inputClass} value={String(item.name ?? '')} onChange={(e) => update({ ...content, certifications: content.certifications.map((value, i) => i === index ? { ...value, name: e.target.value } : value) })} /></label><label className="mt-3 block text-sm font-medium">Issuer<input className={inputClass} value={String(item.issuer ?? '')} onChange={(e) => update({ ...content, certifications: content.certifications.map((value, i) => i === index ? { ...value, issuer: e.target.value } : value) })} /></label></div>)}</CardContent></Card>
      </div>
